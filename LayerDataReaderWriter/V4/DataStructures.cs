...
namespace LayerDataReaderWriter.V4
{
    ...
    {
        /// <summary>
        /// Gets the unique identifier of the block element, among the block.
        /// </summary>
        ...
        public LayerBlockElement(ushort id, float tx, float ty, float angle, float sx, float sy, byte type, Component[] components)
            : this()
        {
            ID = id;
            Tx = tx;
    ...
    public class LayerBlock
    {
        ...
        /// <param name="flags">The custom flags of the block.</param>
        /// <param name="elements">The contained block elements.</param>
        public LayerBlock(byte identifier, byte difficulty, float width, float height, bool[] flags, LayerBlockElement[] elements)
        {
            if (elements == null)
        ...
        internal LayerBlock(byte identifier, byte difficulty, float width, float height, bool[] flags, Func<LayerBlockElement[]> readData)
        {
            Identifier = identifier;
            Difficulty = difficulty;
            Width = width;
